...
public class JValidator implements Validator {
    ...
    /**
     * 修改 alibaba JValidator 的实现，增加打印类型
     *
     * @param clazz
     * @param method
     * @return
    ...
    private static String generateMethodParameterClassName(Class<?> clazz, Method method) {
        ...
        StringBuffer buffer = (new StringBuffer()).append(clazz.getName()).append("_").append(toUpperMethoName(method.getName())).append("Parameter");
        Class<?>[] parameterTypes = method.getParameterTypes();
        if (null != parameterTypes && parameterTypes.length > 0) {
            Arrays.stream(parameterTypes).forEach(e -> buffer.append("_").append(e.getName()));
        }
